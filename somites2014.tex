...
(XX next bit is crap.  it's repeated from methods, so wasn't sure what to say exactly)
...
This accomplishes smoothing in quite an indirect way, as a competition between different possible class labels for a given pixel, mediated by the normalization Z.  Moreover, this smoothing is completely model agnostic.  We observe that GeoF sometimes generates smooth posteriors across individual segments; however this sensitively depends on the local texture of the image.  See \ref{fig:smoothing}, GeoF, for examples.

\paragraph{MAP Inference. }

Finally, we consider another model-based method of smoothing.  This approach is exteremely similar to ours; however, at every level the MAP solution of the constellation model is used as features to the next layer.  This returns hard decisions about the class of every pixel, see \ref{fig:smoothing}, MAP.

\paragraph{Quantitative Results}

Autocontext is the simplest approach that we consider, and it returns a final Dice score of 0.60 (sd=0.20) after three levels (\ref{tab:results}).  Interestingly, all of the methods that we tried stopped improving after three levels of cascading, presumably due to the small size of the data set.  In contrast to Auto-context, GeoF generates significantly smoother posteriors, and performs better at every level of the cascade (\ref{fig:boxplots}).  The best score obtained by GoeF was 0.66 (sd=0.22) after three levels, when evaluated on its smoothed output.  This increase of 0.06 is comparable to the gains reported in (XX ref GeoF) when applying geodesic smoothing, without additionally changes to the training objective.

The next method that we tried was MAP Inference, which computes the optimal configuration of the constellation model as smooth features into the next level.  MAP Inference performs considerably better than the previous techniques after the first level of the cascade, with a mean Dice Score of XX.  This approach also improves over the cascade, reaching a final Dice Score of 0.76 (sd=0.27) after three levels, when evaluated on the MAP output.

Finally, our approach yielded the highest Dice Score of 0.82 (sd=0.18) after three levels of cascading.  The main difference that we observed between our technique and MAP inference was a dramatic increase in the mean Dice Score and reduction in the standard deviation over the 3 levels, which we account to \'rescuing\' outliers (\ref{fig:rescue}).
